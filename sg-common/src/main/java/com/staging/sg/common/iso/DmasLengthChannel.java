...
/**
 * Channel jPOS au framing identique a DmasNetworkUtil :
 *   2 octets de longueur (big-endian) + message packe.
 * Utilise des deux cotes (serveur acquereur + client issuer).
 */
public class DmasLengthChannel extends BaseChannel {

    public DmasLengthChannel() { super(); }

    public DmasLengthChannel(ISOPackager p) {
        super();
        setPackager(p);
    }

    public DmasLengthChannel(String host, int port, ISOPackager p) {
        super(host, port, p);
    }
