...
public class SwitchLabFraudController {
    ...
    public ResponseEntity<byte[]> proxyGateway(HttpServletRequest request,@RequestHeader HttpHeaders headers,@RequestBody(required=false)byte[] body){
        ...
        String path=request.getRequestURI().substring("/api/switchlab/v1/fraud/gateway".length());return fraud.forwardGateway("/api/fraud-gateway/v1"+path,request.getQueryString(),HttpMethod.valueOf(request.getMethod()),headers,body);
    }}
