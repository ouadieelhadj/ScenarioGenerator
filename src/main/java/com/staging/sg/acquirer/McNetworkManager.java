...
/**
 * Network Manager — Mastercard Acquiring.
 *
 * Startup sequence (auto) :
 *   1. Key Exchange (ZMK + ZPK + ZAK)
 *   2. Sign-on (0800 FC=301)
 *   3. Echo heartbeat (0800 FC=302) every X seconds
 */
@Service
public class McNetworkManager {
    ...
    private long keyExchangeTime = 0;
    ...
    public void init() {
        if (!autoSignon) {
            log.info("[NETWORK] Auto sign-on disabled");
            return;
        }
        scheduler = Executors.newSingleThreadScheduledExecutor(
                r -> new Thread(r, "mc-network-manager"));

        // Wait 2 seconds for ISOServer to be ready
        scheduler.schedule(() -> {
            try {
                // Step 1 : Key Exchange
                log.info("[NETWORK] Step 1 — Key Exchange...");
                McKeyExchangeResult keyResult = keyExchangeManager.exchangeAllKeys();
                if (keyResult.isSuccess()) {
                    keysExchanged.set(true);
                    keyExchangeTime = System.currentTimeMillis();
                    zmkKcv = keyResult.getZmkKcv();
                    zpkKcv = keyResult.getZpkKcv();
                    zakKcv = keyResult.getZakKcv();
                    log.info("[NETWORK] Key Exchange OK — ZMK={} ZPK={} ZAK={}",
                            zmkKcv, zpkKcv, zakKcv);
                } else {
                    log.warn("[NETWORK] Key Exchange failed — {}", keyResult.getMessage());
                    return;
                }

                // Step 2 : Sign-on
                log.info("[NETWORK] Step 2 — Sign-on...");
                McNetworkResult signonResult = sendSignOn();
                if (signonResult.isSuccess()) {
                    log.info("[NETWORK] Sign-on OK — session established");
                    startHeartbeat();
                } else {
                    log.warn("[NETWORK] Sign-on failed — {}", signonResult.getMessage());
                }

            } catch (Exception e) {
                log.warn("[NETWORK] Startup failed : {}", e.getMessage());
            }
        }, 2, TimeUnit.SECONDS);
    }
    ...
    public McNetworkResult sendSignOn() throws Exception {
        ...
        log.info("[NETWORK] Sending 0800 Sign-on — STAN={}", stan);
        ...
        log.info("[NETWORK] 0810 Sign-on Response — DE39={}", rc);
    ...
    public McNetworkResult sendEcho() throws Exception {
        ...
        log.debug("[NETWORK] Sending 0800 Echo — STAN={}", stan);
        ISOMsg response = net.sendAndReceive(request, resolveHost(), resolvePort(), timeoutSeconds);
        ...
        log.info("[NETWORK] 0810 Echo Response — DE39={}", rc);
    ...
    private void startHeartbeat() {
        scheduler.scheduleAtFixedRate(() -> {
            try {
                ...
                if (!result.isSuccess()) {
                    log.warn("[NETWORK] Echo failed — retrying sign-on");
                    signedOn.set(false);
                    sendSignOn();
                }
            } catch (Exception e) {
                log.warn("[NETWORK] Heartbeat error : {}", e.getMessage());
                signedOn.set(false);
            }
        }, echoIntervalSeconds, echoIntervalSeconds, TimeUnit.SECONDS);
        log.info("[NETWORK] Heartbeat started — interval {}s", echoIntervalSeconds);
    }
