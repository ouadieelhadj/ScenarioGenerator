...
wait_http "$SIMULATOR_URL/api/ecommerce-simulator/v1/health" ecommerce-simulator
...
python - "$transaction_id" "$profile_id" "$payment_pan" \
  "$payment_expiry" "$ISSUING_E2E_CURRENCY" "$amount_minor" "$ROUTE" <<'PY' |
import json,sys
tx,profile,pan,expiry,currency,amount,route=sys.argv[1:]
...
PY
curl -fsS -X POST "$SIMULATOR_URL/api/ecommerce-simulator/v1/purchases" \
...
if result.get("authenticationStatus") != "NOT_PERFORMED":
...
print("[ECOM E2E] ACHAT APPROUVE - route="+sys.argv[2]+", RC=00, 3DS=NOT_PERFORMED, auth="+str(result.get("authorizationCode")))
PY
