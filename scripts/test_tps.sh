...
for i in $(seq 1 95); do
  ...
  AVG_TPS=$(echo "$RESPONSE" | grep -o '"avgTps":[0-9.]*' | grep -o '[0-9.]*')
  AVG_MS=$(echo "$RESPONSE" | grep -o '"avgResponseMs":[0-9.]*' | grep -o '[0-9.]*')
...
P95=$(echo "$RESPONSE" | grep -o '"p95ResponseMs":[0-9.]*' | grep -o '[0-9.]*')
P99=$(echo "$RESPONSE" | grep -o '"p99ResponseMs":[0-9.]*' | grep -o '[0-9.]*')
STATUS=$(echo "$RESPONSE"   | grep -o '"status":"[^"]*"'       | cut -d'"' -f4)
