...
/**
 * Bootstrap KEK côté EMETTEUR.
 * Reçoit la KEK claire, la forme sous le LMK local (émetteur),
 * et upsert la colonne kek_under_iss_lmk dans dmas_kek.
...
public class KekBootstrapController {
    ...
    public KekBootstrapController(JposHsmService hsm, DmasKekRepository kekRepo) {
        this.hsm = hsm;
    ...
    public ResponseEntity<?> bootstrap(@RequestBody Map<String,String> body) {
        try {
            ...
            DmasKek kek = kekRepo.findByMemberGroupId(mgid).orElseGet(DmasKek::new);
            ...
            if (kek.getDescription() == null) kek.setDescription("KEK bootstrap (issuer side)");
            kekRepo.save(kek);
            ...
            log.info("[KEK-BOOT] Issuer — group={} KCV={}", mgid, formed.kcv);
